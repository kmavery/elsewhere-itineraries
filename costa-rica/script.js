...
if (mapCard && routeCards.length) {
  const setActiveRoute = (route) => {
    mapCard.dataset.activeRoute = route || "all";
    routeCards.forEach((card) => {
      card.classList.toggle("active", Boolean(route) && card.dataset.route === route);
    });
  };
  ...
  routeCards.forEach((card) => {
    card.addEventListener("mouseenter", () => setActiveRoute(card.dataset.route));
    card.addEventListener("focus", () => setActiveRoute(card.dataset.route));
    card.addEventListener("mouseleave", () => setActiveRoute("all"));
    card.addEventListener("blur", () => setActiveRoute("all"));
    card.addEventListener("click", () => {
      const route = mapCard.dataset.activeRoute === card.dataset.route ? "all" : card.dataset.route;
